...
% Load MNIST Data
normalize = 0;
trainX_full = loadMNISTImages('../../dataset/train-images.idx3-ubyte', normalize);
trainY_full = loadMNISTLabels('../../dataset/train-labels.idx1-ubyte');
testX = loadMNISTImages('../../dataset/t10k-images.idx3-ubyte', normalize);
testY = loadMNISTLabels('../../dataset/t10k-labels.idx1-ubyte');
...
% save MNIST_tanh_100_50_DenseNet.mat net;
load MNIST_tanh_100_50_DenseNet.mat net;
predY = classify(net,testX)';
accuracy = sum(predY == testY)/length(testY)

N = 300; % get 300 images and its labels from the imdsValidation
IM_data = zeros(784, N);
IM_labels = zeros(N, 1);
...
% figure                                          % initialize figure
% colormap(gray)                                  % set to grayscale
% for i = 1:150                                    % preview first 50 samples
...
% save tanh_100_50_images.mat IM_data IM_labels
IM = [IM_labels IM_data'];
writematrix(IM,'../../data/tanh_100_50_raw.csv');
