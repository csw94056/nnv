...
[trainX_full_orig,trainY_full,testX_orig,testY] = helperCIFAR10Data.load(cifar10Data);
...
digit = [];
for k = 1:size(trainX_full_orig,4)
    digit = reshape(trainX_full_orig(:,:,:,k),[32,32,3]);
%     digit = images(k,:,:,:);
    ...
    B = reshape(C,[3,1024]);
    trainX_full(:,k) = reshape(B,3072,1);
end

digit = [];
for k = 1:size(testX_orig,4);
...
% save ../nets/FNNtest/CIFAR10_FNNtest_tanh.mat net;

load ../nets/FNNtest/CIFAR10_FNNtest_tanh.mat;
...
% writematrix(IM,'../../CIFAR10/data/CIFAR10_FNNtest_tanh_raw.csv');
