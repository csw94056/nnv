...
plot(Se, 'c');
title('input');
legend('AbsDom','RStar AbsDom 2','RStar AbsDom 3', 'Star approx');
[lb, ub] = R2.getRanges;
B = Box(lb, ub);
Z = B.toZono;
for i = 1:steps
...
%     hold on;
    ...
    A = PosLin.reach_absdom(A, 0);
    R2 = PosLin.reach_rstar_absdom_with_two_pred_const(R2, 0);
    R3 = PosLin.reach_rstar_absdom_with_three_pred_const(R3, 0);
    %Re
    Se = PosLin.reach_star_approx2(Se);
    [lb, ub] = R2.getRanges;
    B = Box(lb, ub);
    Z = B.toZono;
